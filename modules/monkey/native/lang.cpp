...
void gc_collect(){
	...
	int tomark=sweep ? 0x7fffffff : gc_marked_objs+( CFG_CPP_GC_MARK_RATE>gc_alloced_objs ? CFG_CPP_GC_MARK_RATE : gc_alloced_objs );
	...
	if( alloced>CFG_CPP_GC_TRIGGER && GC_LIST_EMPTY( gc_queued_list ) ){
		sweep=true;
	}
